...
public class ResetPassword extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		} else if (!Util.checkStringParam(username)) {
			err += "Tài khoản không tồn tại!";
		} else {
			if (userDAO.checkUser(username) == false) {
				err += "Tài khoản nhập không tồn tại !";
			}
		}
		...
		try {
			if (err.length() == 0) {
				...
				String passwordReset = Encryption.encryption("passwordreset");
				User new_user = new User(u.getUser_id(), username, passwordReset, u.getNgaysinh(), u.getGioitinh(),
